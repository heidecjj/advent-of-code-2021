...
def depth_first_traverse(snail, nodes):
    left, right = snail

    if isinstance(left, int):
        nodes.append((snail, 0))
    else:
        depth_first_traverse(left, nodes)

    if isinstance(right, int):
        nodes.append((snail, 1))
    else:
        depth_first_traverse(right, nodes)
...
def solve(lines):
    ...
    answer = reduce(snail_add, snails)
    return snail_magnitude(answer)
